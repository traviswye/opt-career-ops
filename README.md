...
| Triage 2,400 listings down to ~30 worth evaluating | **~$2** (Haiku 4.5, 12-job chunks) | No triage stage — upstream users **manually browse and curate** career pages to identify the ~30 worth evaluating. This is free in dollars but typically takes hours of browsing per session. The fork's $2 Haiku pass automates that curation step. *(For context: running the upstream monolithic eval on all 2,400 instead of curating manually would cost ~$1,400–3,600 — which is exactly why manual curation exists in the upstream workflow.)* |
