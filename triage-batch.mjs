...
Run low-cost triage scoring against normalized JD artifacts.

Usage:
  node triage-batch.mjs [options]
...
Options:
  ...
  --parallel N       Concurrent workers (default: ${DEFAULT_PARALLEL})
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    switch (arg) {
      ...
      case '--parallel':
        ...
        break;
      case '--chunk-size':
        args.chunkSize = Math.max(1, Number.parseInt(argv[++i], 10) || 1);
        break;
      case '--model':
...
function collectJobFiles(inputPath) {
  ...
  const statType = extname(inputPath).toLowerCase();
  if (statType === '.json') {
  ...
  await runPool(jobChunks, args.parallel, async (jobChunk) => {
    try {
      ...
      for (const outcome of chunkOutcomes) {
        ...
        console.log(`${outcome.status === 'cached' ? '↺' : '✓'} ${outcome.result.job_file}`);
      }
      console.log(`${outcome.status === 'cached' ? '↺' : '✓'} ${jobFile}`);
    } catch (error) {
